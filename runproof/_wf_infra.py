...
def pre_commit_hook_text(root: Path) -> str:
    ...
    return "\n".join(
        [
            "#!/bin/sh",
            "# Generated by RunProof. Edit with care.",
            command,
            "",
        ]
    )
...
def install_hooks(root: Path) -> list[Finding]:
    ...
    try:
        ...
        pass
    print(f"Installed RunProof pre-commit hook: {pre_commit.as_posix()}")
    ...
    try:
        ...
        pass
    print(f"Installed RunProof pre-push hook: {pre_push.as_posix()}")
...
name: RunProof Governance Guard
...
jobs:
  runproof-guard:
    name: RunProof governance check
    ...
    steps:
      ...
      - name: Install RunProof
        run: pip install runproof-cli

      - name: RunProof governance guard
        run: runproof guard --root . --strict-state

      # Uncomment to require passing execution evidence for verified changes:
      # - name: RunProof evidence guard
      #   run: runproof guard --root . --strict-state --require-execution-evidence
"""

_GITLAB_CI_TEMPLATE = """\
runproof-governance-guard:
  stage: test
  image: python:3.11-slim
  script:
    - pip install runproof-cli
    - runproof guard --root . --strict-state
  # Uncomment to require passing execution evidence:
  # - runproof guard --root . --strict-state --require-execution-evidence
"""

_CI_TEMPLATES: dict[str, tuple[str, str]] = {
    "github-actions": (_GITHUB_ACTIONS_TEMPLATE, ".github/workflows/runproof-guard.yml"),
    "gitlab-ci":      (_GITLAB_CI_TEMPLATE,       ".gitlab-ci-runproof.yml"),
}
