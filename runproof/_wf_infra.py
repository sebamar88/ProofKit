...
def pre_commit_hook_text(root: Path) -> str:
    ...
    command = f"proofkit guard --root {shlex.quote(root_arg)} --require-active-change --strict-state"
    return "\n".join(
        [
            "#!/bin/sh",
            "# Generated by ProofKit. Edit with care.",
            command,
            "",
        ]
    )
...
def pre_push_hook_text(root: Path) -> str:
    root_arg = root.as_posix()
    command = f"proofkit guard --root {shlex.quote(root_arg)} --strict-state"
    return "\n".join(
        [
            "#!/bin/sh",
            "# Generated by ProofKit. Edit with care.",
...
def install_hooks(root: Path) -> list[Finding]:
    ...
    try:
        ...
        pass
    print(f"Installed ProofKit pre-commit hook: {pre_commit.as_posix()}")
    ...
    try:
        ...
        pass
    print(f"Installed ProofKit pre-push hook: {pre_push.as_posix()}")
  ...
  sdd-guard:
    ...
    steps:
      ...
      - name: Install ProofKit
        run: pip install proofkit-cli

      - name: SDD governance guard
        run: proofkit guard --root . --strict-state

      # Uncomment to require passing execution evidence for verified changes:
      # - name: SDD evidence guard
      #   run: proofkit guard --root . --strict-state --require-execution-evidence
"""
...
sdd-governance-guard:
  ...
  script:
    - pip install proofkit-cli
    - proofkit guard --root . --strict-state
  # Uncomment to require passing execution evidence:
  # - proofkit guard --root . --strict-state --require-execution-evidence
"""
